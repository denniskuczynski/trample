...
The above configuration will start 5 (concurrency) sessions with a delay of 1 second between each session by logging in as a random user at the url in the login block. Then, it'll hit the two urls underneath it 10 (iterations) times during each session.
